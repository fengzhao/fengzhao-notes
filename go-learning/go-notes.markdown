...
golang中的`nil`，很多人都误以为与 Java、PHP 等编程语言中的null一样，但是实际上复杂很多。
...
// 使用make声明一个map(key的类型是 string，value的类型是 int)
...
// 判断map中是否存在某个key，如果存在则修改
// 在其他语言中，判断值通常是三步：
// 1.声明一个变量，并初始化为空
// 2.从map中试图取值并赋值给该变量
// 3.判断该变量是否为空，如果为空则表示 map 中没有包含该变量
...
// 迭代map，与切片和数组一样，只不过 range 返回的不是索引和值，而是键值对。
...
// 结构体里每个字段都会用一个已知类型声明。这个已知类型可以是内置类型，也可以是其他用户定义的类型
type user struct {
	name       string
	email      string
	ext        int
