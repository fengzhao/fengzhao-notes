...
// 导包规范：有顺序的引入包, 不同的类型采用空格分离, 第一种是标准库, 第二种是项目包, 第三种是第三方包.
import (
    "encoding/json"
    "strings"
...
在 golang 种，导入包却不使用，和函数内声明变量但是并不使用一样，都会在编译错误。
...
这样的init初始化函数除了不能被调用或引用外，其他行为和普通函数类似。在每个文件中的 init 初始化函数，在程序开始执行时按照它们声明的顺序被自动调用。
...
```go
// 无符号整型
uint8       // 8位二进制，the set of all unsigned  8-bit integers (0 to 255)
uint16      // 16位二进制，the set of all unsigned 16-bit integers (0 to 65535)
uint32      // the set of all unsigned 32-bit integers (0 to 4294967295)
uint64      // the set of all unsigned 64-bit integers (0 to 18446744073709551615)
// byte类型一般用于强调数值是一个原始的数据而不是一个小的整数。
byte        alias for uint8
// rune类型通常用于表示一个Unicode码点
