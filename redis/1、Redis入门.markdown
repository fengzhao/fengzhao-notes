...
- **期限信息的设置是保存在磁盘中持久存在的**，这意味着（当 redis 服务停止的时候，时间也是在走的，在redis持久化中很详细讲）

```shell
127.0.0.1:6379> set key some-value
OK
# expire key time 设置过期时间
127.0.0.1:6379127.0.0.1:6379> expire key 5
(integer) 1
# 立即取值，key还存在
...
> redis 挂了？请求也是全部打到数据库
>
> 针对第一种情况，可以实现 redis 高可用：redis cluster 或 redis sentinel 模式。
>
> **雪崩的关键问题在于：大量的 key 都在同一时间失效，如果这时候海量并发， 请求都会打到数据库上。这时候数据库压力很大，导致系统崩溃**
