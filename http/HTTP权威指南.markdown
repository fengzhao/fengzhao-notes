...
在设计安全方案 时，也要以这三个要素为基本的出发点，去全面地思考所要面对的问题。
...
从以前的信件，无线电报，现在的互联网通信等，都有可能被截取或被篡改。所以通讯加密这个需求自然而然就诞生了。
...
而且如果用 HTTP 协议进行通信，HTTP 本身没有加密功能，所以也无法做到对通信整体(使用 HTTP 协议通信的请求和响应的内容)进行加密。
