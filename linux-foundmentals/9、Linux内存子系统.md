...
我们通常所说的内存容量，比如常常说的某笔记本（某服务器内存容量）8GB，16GB，其实指的是物理内存。**物理内存也称为主存，大多数计算机用的主存都是动态随机访问内存（DRAM）。**
...
那么提供的可寻址物理地址范围为2^32=4GB（**在这里要注意一点，我们平常所说的32位CPU和64位CPU指的是CPU一次能够处理的数据宽度，即位宽，不是地址总线的数目**）。
