...
```shell
# 设置主机名 
hostnamectl set-hostname master   # 192.168.20.2
hostnamectl set-hostname slave1   # 192.168.20.3
hostnamectl set-hostname slave2   # 192.168.20.4
...
CREATE USER 'mharep'@'192.168.20.%' IDENTIFIED WITH 'mysql_native_password' by 'admin@123' ;
grant replication slave on *.* to mharep@'192.168.20.%' ;
...
```shell
# 下载并安装MHA相关RPM包：mha-manager(master节点)  mha-node(所有节点)
