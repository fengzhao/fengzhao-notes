...
GUI是一种包含图形元素（例如窗口、图标和按钮）的用户界面。该术语诞生于 1970 世纪 XNUMX 年代，用于区分图形界面和基于文本的界面（例如命令行界面）。然而，如今几乎所有数字界面都是 GUI。



在20世纪70年代末，计算机还是技术爱好者和专家的领域，但乔布斯敏锐地意识到，**通过简化人机交互**界面，计算机可以进入每一个家庭。

他推动了图形用户界面（GUI）、鼠标和键盘的创新，使得普通人也能轻松使用电脑。

这一里程碑式的贡献不仅推动了个人计算机的发展，也为未来的智能设备和软件设计奠定了基础。



多年来，图形用户界面 (GUI) 仅由鼠标和键盘控制。这类输入设备非常适合台式电脑，但在智能手机和平板电脑等移动设备上效果不佳。

因此，移动操作系统被设计为使用触摸屏界面。像苹果 iOS 这样的移动操作系统增加了更大的图标，并支持滑动和捏合等触摸命令来放大或缩小。

汽车界面通常设计为通过旋钮和按钮控制，而电视界面则设计为通过遥控器控制。无论输入类型如何，这些界面都被视为 GUI，因为它们包含图形元素。
...
Tui 程序是一种运行在终端环境下的用户界面程序，全称为 Text-based User Interface（基于文本的用户界面）

与传统的纯命令行工具不同，Tui 程序能够在保持命令行高效性的同时，提供更加直观、结构化的界面布局

它通常通过字符图形的方式绘制边框、菜单、列表、状态栏等界面元素，使用户可以在不依赖图形桌面环境的情况下，获得接近图形界面的操作体验。

这类程序广泛应用于服务器管理、系统监控、日志查看、文件操作等场景，尤其适合远程终端操作或资源受限的环境

Tui 程序支持键盘快捷键、方向键导航、菜单选择等交互方式，提升了操作效率和用户体验。例如，你可以使用快捷键快速切换面板、执行命令或查看帮助，而无需记忆复杂的命令参数。



常见的 Tui 工具包括 `htop`（系统资源监控）、`ranger`（终端文件管理器）、`gitui`（Git 命令行界面）等。

它们不仅功能强大，而且响应迅速，是开发者和系统管理员的得力助手。通过结合命令行的强大能力与简洁的界面设计，Tui 程序在现代终端应用中占据着重要地位，是连接纯文本命令与图形化操作之间的桥梁。

**==在图形界面(GUI)统治世界的今天，文本用户界面(TUI)正在悄然复兴。从`htop`的系统监控到`ncdu`的磁盘分析，从`lazygit`的Git操作到现代DevOps工具，TUI以其轻量、快速、可脚本化的特点，重新赢得了开发者和高级用户的青睐。==**
...
- declare
  - declare命令是bash的一个内建命令，它可以用来声Shell变量，设置变量的属性（Declare variables and/or give them attributes）。
  - 该命令也可以写作typeset。虽然人们很少使用这个命令，如果知道了它的一些用法，就会发现这个命令还是挺有用的。
- let 
- local 
  - 声明函数局部变量（bash/ksh/zsh）
...
```shell
在引用变量的时候，一般用 ${VIRABLE_NAME}，这是一个好习惯。用大括号把 $ 符号后面的变量名括起来。因为用括号会比较精确的界定变量名称的范围。
```
...
最著名的操作系统变量就是 `PATH` 。在 Windows 和 Linux 中都存在这个环境变量。它表示在命令行中执行命令时的查找路径。

在 Linux 命令行中，可以通过 `echo ${VARIABLENAME}` 命令来打印变量值。
...
同样，在调用函数时也可以传递参数。**Shell 函数参数的传递和其它编程语言不同，没有所谓的形参和实参，在定义函数时也不用指明参数的名字和数目这些。**

换句话说，定义 Shell 函数时不能带参数，但是在调用函数时却可以传递参数，这些传递进来的参数，在函数内部就也使用`$n`的形式接收，例如，`$1` 表示第一个参数，`$2` 表示第二个参数，依次类推。

这种通过`$n`的形式来接收的参数，在 Shell 中称为**位置参数**。



**在讲解变量的命名时，前面提到：变量的名字必须以字母或者下划线开头，不能以数字开头；但是位置参数却偏偏是数字，这和变量的命名规则是相悖的，所以我们将它们视为“特殊变量”。**
...
| `${@}` | 表示所有的位置参数（即传给脚本或函数的所有参数）。当被双引号包裹时 `"$@"`，会将每个参数作为独立的字符串处理（常用来遍历所有参数，能正确处理带空格的参数） |
| `${?}` | 上一个命令的退出状态码（0 表示成功，非0失败）                |
...
我们大多数天天在 `Bash` 下无意识中使用着 `Emacs` 键绑定类型，即使很多人可能从未用过 `Emacs` 本身。比如我们在 `Bash` 下的按键组合：
...
`bash-completion` 是一个用于 **Bash shell** 的可编程自动补全框架。它的核心思想是允许开发者为任何命令、脚本或程序编写一个**自动补全脚本**，从而大大提升命令行操作的效率和准确性。

在bash中一般都是使用`bash-comletion`来自动补全，很多Linux发行版本都有自带这个包。
...
还有人记得 IPython Shell 么？曾经想利用 Python 的便利性在 Shell 领域代替 bash/zsh，Python 当然是一门足够强大的通用语言，但在 shell 这个领域，它怎么着都干不过 bash/zsh ，为啥呢？

因为 bash/zsh 就是为了操作各种命令和文件而设计的，命令/文件是里面的一等公民，最方便最有力的表达方式都让给了命令以及文件操作，那么做这些事情 bash/zsh 当让比 python 的描述能力更强，更清晰。
