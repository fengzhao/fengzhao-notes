...
由于 TCP 的特性，包括初次连接的握手、连接后窗口大小的调整、连接后分配的默认缓存（128KB Linux），使得每一路 TCP 的开销都不小。

OpenSSH（5.6版本上）有一项特性叫作SSH 多路复用（SSH multiplexing），也称长连接保持（ControlPersist）

多路复用是在**同一个客户端**与服务器之间，复用**同一个 TCP 长连接**。

当使用SSH多路复用连接同一个服务器的SSH会话时将会共享同一条TCP连接，这样消耗在TCP建立连接的时间就只会发生在第一次连接的时候。



当启用了多路复用

- 首次客户端SSH到服务器时，客户端的SSH将启动主连接，客户端会创建一个UNIX 套接字(控制套接字，Control socket)维持与远程服务器的连接;
- 当下次SSH服务器时，OpenSSH将会使用已建立的控制套接字与服务器通信，而不再是重新建立新的TCP连接，减少了重复建立tcp连接的开销;
...
```bash
# 作用于所有远程主机（* 是通配符，你也可以指定特定的 IP 或域名）
Host * # 强制使用 SSH 协议版本 2（版本 1 已不安全且过时）
  Protocol 2  

  # 心跳间隔（秒）：每隔 60 秒，客户端向服务端发送一个“心跳”信号
  # 作用：防止防火墙或路由器因为连接长时间空闲而强行断开你的 SSH
  ServerAliveInterval 60

  # 最大重试次数：如果服务端连续 30 次没有响应心跳，客户端才主动断开
  # 结合上面一行：60秒 * 30次 = 30分钟。只要半小时内网络恢复，连接就不会断
  ServerAliveCountMax 30

  # 开启 SSH 多路复用：
  # auto 表示自动模式。第一次连接时建立 Master 会话，后续连接直接复用它
  ControlMaster auto

  # 链接保持时间：当你关闭所有 SSH 窗口后，Master 连接仍在后台保持 10 分钟
  # 作用：10 分钟内再次登录同一台机器，无需重新输入密码/握手，实现秒连
  ControlPersist 10m

  # 指定多路复用的 Socket 文件存放路径
  # %r 用户名, %h 主机, %p 端口。
  # 注意：如果路径过长（超过108字符）会导致连接失败，建议尽量放在 /tmp/ 下
  ControlPath ~/.ssh/connect-%r@%h:%p.socket  

  # 关闭数据压缩：
  # 在内网或高带宽环境下关闭压缩可以略微降低 CPU 负载。如果在极慢的公网建议设为 yes
  Compression no  

  # 关闭 TCP 层面的保活机制：
  # 因为上面配置了 ServerAlive 这种应用层的保活，所以通常可以关掉 TCP 层的
  # 作用：避免某些严格的防火墙误判 TCP KeepAlive 包为异常攻击
  TCPKeepAlive no 

  # 开启 SSH 代理转发：
  # 允许你在登录远程服务器后，继续使用本地的 SSH Key 去访问第三台机器（如 Git）
  # 作用：不需要把私钥拷贝到远程服务器上，非常安全且方便
  ForwardAgent yes
```
