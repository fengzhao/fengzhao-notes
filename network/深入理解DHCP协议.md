...
DHCP是一种应用层协议，建立在 UDP 协议之上。通常被应用在大型的局域网络环境中。
...
连接到Internet的每台计算机需要在发送或接收数据报文前知道自己的IP地址



早期，具有本地磁盘的系统引导时，计算机一般是从磁盘上的配置文件中读取 I P地址。但是无盘机，如X终端或无盘工作站，则需要采用其他方法来获得 IP 地址。
...
另外，计算机还需要其他信息，如路由器的地址、使用的子网掩码和DNS名字服务器的地址等。可以说这就是上古时代的DHCP协议了。



在 DHCP 协议（1993 年发布）普及之前，局域网内设备的 IP 分配主要依赖静态配置或 RARP 协议。

对于大量同质化设备（如机房电脑、终端机），手动配置静态 IP 的管理成本极高，而 RARP 通过 “MAC 绑定 IP” 的方式，实现了 IP 地址的半自动分配，降低了管理员的配置压力。

例如，学校机房的数十台无盘电脑，通过 RARP 服务器统一分配 IP，避免了 IP 冲突，且配置一次长期有效。
...
| **演进阶段** | **核心协议** | **关键年份** | **核心功能与特点**                                           | **分配模式** | **局限性/改进**                                        |
| ------------ | ------------ | ------------ | ------------------------------------------------------------ | ------------ | ------------------------------------------------------ |
| **初期**     | **RARP**     | 1982         | **核心：MAC → IP** 功能单一，仅能获取 IP 地址。              | **静态映射** | 无法跨网段（仅二层），配置信息极其有限。               |
| **中期**     | **BOOTP**    | 1985         | **扩展配置参数** 支持获取网关、服务器 IP、启动文件名等。     | **静态映射** | **支持跨网段**（通过中继），但仍需手动绑定 MAC 与 IP。 |
| **成熟期**   | **DHCP**     | 1993         | **全量配置，多种模式** 引入租约机制，支持 DNS、NTP 等海量参数。 | **动态分配** | 彻底解决了 IP 资源回收与大规模移动办公的配置难题。     |
