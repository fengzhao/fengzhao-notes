...
在网络中不论传输什么数据，最终底层都需要通过物理介质发送二进制数据，类似于0101的Bit流。纯文本（字符串）中文通常采用UTF-8编码，英文用ASCII编码；
...
**消息体**的字符编码是Content-Type的一个可选属性，加上编码的类型就变成了：
...
一般来说标准并没有规定在不存在charset的时候如何理解字符编码，具体实现上也有很大差异，某些Content-Type不是字符类型数据，自然也没有什么字符编码。

不过某些Content-Type则有默认的字符编码，例如application/x-www-form-urlencoded一般默认为UTF-8编码，application/json一般默认为UTF-8编码等等。
