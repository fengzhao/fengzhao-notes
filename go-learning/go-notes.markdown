...
即  \go\src\net 是一个文件夹，是一个包，这个包名叫 net  ，net 目录里面 go 文件都要声明 package net 
...
go 编译器读到一个 import 关键字导包的时候，按照上述顺序导入，一旦找到满足要求的，就停止查找。
...
数组是Go语言编程中最常用的数据结构之一，在 go 语言内部，数组是一个**长度固定**的数据类型，用于存储一段具有相同的类型的元素的连续块。
