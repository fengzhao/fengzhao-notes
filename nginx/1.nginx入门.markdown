...
# 如果打算用源码的高版本openssl，那编译时候用./configure --with-http_ssl_module --with-openssl=/tmp/openssl-1.0.2k
...
**重定向**：重定向是用户请求服务端后，服务端向客户端发送HTTP 301、302（303、304、307、308），告诉客户端需要去尝试另一个URL。
